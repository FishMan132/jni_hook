...
static void relocateInstructionInArm(uint32_t target_addr, uint32_t *orig_instructions, int length,
    ...
    for (orig_pos = 0; orig_pos < length / sizeof(uint32_t); orig_pos++) {
        ...
        if (type == BLX_ARM || type == BL_ARM || type == B_ARM || type == BX_ARM) {
            ...
            trampoline_instructions[trampoline_pos++] =
                    0xE51F0000 | (r << 12);    // LDR Rr, [PC],其实就是把value的值给Rr
    ...
    for (testloop = 0; testloop <= trampoline_pos; testloop++) {
        LOGE("-新指令--trampoline_instructions[%x]=%x----", testloop,
             trampoline_instructions[testloop]);
    }
}


static void doInlineHook(struct inlineHookItem *item) {
    mprotect((void *) PAGE_START(CLEAR_BIT0(item->target_addr)), PAGE_SIZE,
...
void SnowInlineHook(unsigned int orgfun, void *newfun, void **result) {
    ...
    LOGE("--原函数地址-item->target_addr=%x-新函数地址item->new_addr=%x--", item->target_addr,
         item->new_addr);
    ...
    if (item->target_addr % 4 == 1) {
        LOGE("---------是thumb指令-------");
        relocateInstructionInThumb(item->target_addr - 1, (uint16_t *) item->orig_instructions,
                                   item->length, (uint16_t *) item->trampoline_instructions);
    } else {
        LOGE("---------是arm指令-------");
        relocateInstructionInArm(item->target_addr, (uint32_t *) item->orig_instructions,
                                 item->length, (uint32_t *) item->trampoline_instructions);
    }
    doInlineHook(item);
}
